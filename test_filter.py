...
class TestHbfUS(unittest.TestCase):
    ...
    def test_supersampled_response(self):
        ...
        x[1] = 32
        x[4] = -26
        h = [10, 0, 30, 0, 70, 1, 70, 0, 30, 0, 10]
        self.hbf = SuperHbfUS(h)
        y_sim = self.run_hbf_ss_sim(x, in_samples)[((2 + (len(h)+1)//4) * 2):].real.astype('int').tolist()
        x_zs = np.zeros(in_samples*2).real.astype('int').tolist()
        for i in range(in_samples):
            x_zs[i*2]=x[i]
        y_model = np.convolve(x_zs, h)[:-len(h)+1-((2 + (len(h)+1)//4) * 2)].real.astype('int').tolist()
    ...
    def test_normal_response(self):
        ...
        h = [10, 0, 30, 0, 70, 1, 70, 0, 30, 0, 10]
        self.hbf = SuperHbfUS(h)
        y_sim = self.run_hbf_norm_sim(x, in_samples)[((1 + (len(h) + 1) // 4) * 2) + 1:].real.astype('int').tolist()
        x_zs = np.zeros(in_samples * 2).real.astype('int').tolist()
        for i in range(in_samples):
            x_zs[i * 2] = x[i]
        y_model = np.convolve(x_zs, h)[:-len(h) - ((1 + (len(h) + 1) // 4) * 2)].real.astype('int').tolist()
        self.assertEqual(y_sim, y_model)
